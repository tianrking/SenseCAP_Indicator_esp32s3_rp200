...
typedef struct {
    ...
    float atomic_mass;
    const char *config_str; // Electron configuration string (Linear format)
    const char *category;   // Element category
    const char *melting_point_k; // Melting point in Kelvin (string for flexibility)
    const char *boiling_point_k; // Boiling point in Kelvin (string for flexibility)
    const char *density_g_cm3;   // Density in g/cm³ (string for flexibility)
} element_info_t;

// Data for the first 44 elements (H to Ru) with added physical properties
// Note: Data is approximate and uses standard conditions where applicable. "N/A" used where not applicable/well-defined.
// Density for gases is at STP (0°C, 1 atm) or standard state.
static const element_info_t elements[MAX_ELEMENTS] = {
 // Sym Name        e-, Shells[1-5], Mass,   Config Str, Category,         Melt(K), Boil(K), Density(g/cm³)
 { "H", "Hydrogen",  1,{1,0,0,0,0},  1.008, "1s1",      "Nonmetal",       "13.99", "20.27", "0.000089 (Gas)"},
 { "He","Helium",    2,{2,0,0,0,0},  4.0026,"1s2",      "Noble Gas",      "N/A",   "4.22",  "0.000178 (Gas)"},
 { "Li","Lithium",   3,{2,1,0,0,0},  6.94,  "[He] 2s1", "Alkali Metal",   "453.65","1615",  "0.534"},
 { "Be","Beryllium", 4,{2,2,0,0,0},  9.0122,"[He] 2s2", "Alkaline Earth", "1560",  "2742",  "1.85"},
 { "B", "Boron",     5,{2,3,0,0,0},  10.81, "[He] 2s2 2p1","Metalloid",    "2349",  "4200",  "2.34"},
 { "C", "Carbon",    6,{2,4,0,0,0},  12.011,"[He] 2s2 2p2","Nonmetal",     "3800",  "5100",  "2.267 (Graphite)"},
 { "N", "Nitrogen",  7,{2,5,0,0,0},  14.007,"[He] 2s2 2p3","Nonmetal",     "63.15", "77.36", "0.00125 (Gas)"},
 { "O", "Oxygen",    8,{2,6,0,0,0},  15.999,"[He] 2s2 2p4","Nonmetal",     "54.36", "90.19", "0.00142 (Gas)"},
 { "F", "Fluorine",  9,{2,7,0,0,0},  18.998,"[He] 2s2 2p5","Halogen",      "53.48", "85.03", "0.00169 (Gas)"},
 { "Ne","Neon",     10,{2,8,0,0,0},  20.180,"[He] 2s2 2p6","Noble Gas",    "24.56", "27.10", "0.00090 (Gas)"},
 { "Na","Sodium",   11,{2,8,1,0,0},  22.990,"[Ne] 3s1", "Alkali Metal",   "370.94","1156",  "0.968"},
 { "Mg","Magnesium",12,{2,8,2,0,0},  24.305,"[Ne] 3s2", "Alkaline Earth", "923",   "1363",  "1.738"},
 { "Al","Aluminum", 13,{2,8,3,0,0},  26.982,"[Ne] 3s2 3p1","Post-trans M.", "933.47","2792",  "2.70"},
 { "Si","Silicon",  14,{2,8,4,0,0},  28.085,"[Ne] 3s2 3p2","Metalloid",    "1687",  "3538",  "2.33"},
 { "P", "Phosphorus",15,{2,8,5,0,0}, 30.974,"[Ne] 3s2 3p3","Nonmetal",     "317.3", "553.7", "1.823 (White)"},
 { "S", "Sulfur",   16,{2,8,6,0,0},  32.06, "[Ne] 3s2 3p4","Nonmetal",     "388.36","717.8", "2.07"},
 { "Cl","Chlorine", 17,{2,8,7,0,0},  35.45, "[Ne] 3s2 3p5","Halogen",      "171.6", "239.11","0.00321 (Gas)"},
 { "Ar","Argon",    18,{2,8,8,0,0},  39.948,"[Ne] 3s2 3p6","Noble Gas",    "83.81", "87.30", "0.00178 (Gas)"},
 { "K", "Potassium",19,{2,8,8,1,0},  39.098,"[Ar] 4s1", "Alkali Metal",   "336.7", "1032",  "0.862"},
 { "Ca","Calcium",  20,{2,8,8,2,0},  40.078,"[Ar] 4s2", "Alkaline Earth", "1115",  "1757",  "1.55"},
 { "Sc","Scandium", 21,{2,8,9,2,0},  44.956,"[Ar] 3d1 4s2","Transition M.", "1814",  "3109",  "2.985"},
 { "Ti","Titanium", 22,{2,8,10,2,0}, 47.867,"[Ar] 3d2 4s2","Transition M.", "1941",  "3560",  "4.506"},
 { "V", "Vanadium", 23,{2,8,11,2,0}, 50.942,"[Ar] 3d3 4s2","Transition M.", "2183",  "3680",  "6.11"},
 { "Cr","Chromium", 24,{2,8,13,1,0}, 51.996,"[Ar] 3d5 4s1","Transition M.", "2180",  "2944",  "7.19"},
 { "Mn","Manganese",25,{2,8,13,2,0}, 54.938,"[Ar] 3d5 4s2","Transition M.", "1519",  "2334",  "7.21"},
 { "Fe","Iron",     26,{2,8,14,2,0}, 55.845,"[Ar] 3d6 4s2","Transition M.", "1811",  "3134",  "7.874"},
 { "Co","Cobalt",   27,{2,8,15,2,0}, 58.933,"[Ar] 3d7 4s2","Transition M.", "1768",  "3200",  "8.90"},
 { "Ni","Nickel",   28,{2,8,16,2,0}, 58.693,"[Ar] 3d8 4s2","Transition M.", "1728",  "3186",  "8.908"},
 { "Cu","Copper",   29,{2,8,18,1,0}, 63.546,"[Ar] 3d10 4s1","Transition M.", "1357.7","2835",  "8.96"},
 { "Zn","Zinc",     30,{2,8,18,2,0}, 65.38, "[Ar] 3d10 4s2","Post-trans M.", "692.68","1180",  "7.14"},
 { "Ga","Gallium",  31,{2,8,18,3,0}, 69.723,"[Ar] 3d10 4s2 4p1","Post-trans M.", "302.91","2673",  "5.91"},
 { "Ge","Germanium",32,{2,8,18,4,0}, 72.63, "[Ar] 3d10 4s2 4p2","Metalloid",    "1211.4","3106",  "5.323"},
 { "As","Arsenic",  33,{2,8,18,5,0}, 74.922,"[Ar] 3d10 4s2 4p3","Metalloid",    "1090 (Subl.)","887 (Subl.)","5.727"},
 { "Se","Selenium", 34,{2,8,18,6,0}, 78.971,"[Ar] 3d10 4s2 4p4","Nonmetal",     "494",   "958",   "4.81 (Gray)"},
 { "Br","Bromine",  35,{2,8,18,7,0}, 79.904,"[Ar] 3d10 4s2 4p5","Halogen",      "265.8", "332.0", "3.12 (Liquid)"},
 { "Kr","Krypton",  36,{2,8,18,8,0}, 83.798,"[Ar] 3d10 4s2 4p6","Noble Gas",    "115.78","119.93","0.00374 (Gas)"},
 { "Rb","Rubidium", 37,{2,8,18,8,1}, 85.468,"[Kr] 5s1", "Alkali Metal",   "312.45","961",   "1.532"},
 { "Sr","Strontium",38,{2,8,18,8,2}, 87.62, "[Kr] 5s2", "Alkaline Earth", "1050",  "1655",  "2.64"},
 { "Y", "Yttrium",  39,{2,8,18,9,2}, 88.906,"[Kr] 4d1 5s2","Transition M.", "1799",  "3609",  "4.472"},
 { "Zr","Zirconium",40,{2,8,18,10,2},91.224,"[Kr] 4d2 5s2","Transition M.", "2128",  "4682",  "6.52"},
 { "Nb","Niobium",  41,{2,8,18,12,1},92.906,"[Kr] 4d4 5s1","Transition M.", "2750",  "5017",  "8.57"},
 { "Mo","Molybdenum",42,{2,8,18,13,1},95.96, "[Kr] 4d5 5s1","Transition M.", "2896",  "4912",  "10.28"},
 { "Tc","Technetium",43,{2,8,18,13,2},(98), "[Kr] 4d5 5s2","Transition M.", "2430",  "4538",  "11.5"},
 { "Ru","Ruthenium",44,{2,8,18,15,1},101.07,"[Kr] 4d7 5s1","Transition M.", "2607",  "4423",  "12.45"}
};
...
static lv_obj_t * label_melt_lbl; // New label
static lv_obj_t * label_melt_val; // New label
static lv_obj_t * label_boil_lbl; // New label
static lv_obj_t * label_boil_val; // New label
static lv_obj_t * label_density_lbl;// New label
static lv_obj_t * label_density_val;// New label
// State
...
static void electron_anim_exec_cb(void * var, int32_t v) { lv_obj_t * obj = (lv_obj_t *)var; if(!lv_obj_is_valid(obj)) return; lv_coord_t radius = (lv_coord_t)(lv_uintptr_t)lv_obj_get_user_data(obj); float angle_rad = (float)(v % 360) * M_PI / 180.0f; lv_coord_t x = center_x + (lv_coord_t)(radius * cosf(angle_rad)) - ELECTRON_RADIUS; lv_coord_t y = center_y + (lv_coord_t)(radius * sinf(angle_rad)) - ELECTRON_RADIUS; lv_obj_set_pos(obj, x, y); }
/** @brief Hide object after fade out */
static void fade_out_anim_ready_cb(lv_anim_t *a) { lv_obj_t * obj = (lv_obj_t *)a->var; if (lv_obj_is_valid(obj)) lv_obj_add_flag(obj, LV_OBJ_FLAG_HIDDEN); }
/** @brief Start fade out animations */
static void start_fade_out_old_element(void) { lv_anim_t a; lv_anim_init(&a); lv_anim_set_time(&a, FADE_TIME_MS); lv_anim_set_exec_cb(&a, fade_anim_opa_exec_cb); lv_anim_set_values(&a, LV_OPA_COVER, LV_OPA_TRANSP); lv_anim_set_ready_cb(&a, fade_out_anim_ready_cb); lv_anim_set_deleted_cb(&a, NULL); for (int i = 0; i < MAX_ELECTRONS; i++) { if (electron_objs[i] && lv_obj_is_valid(electron_objs[i]) && !lv_obj_has_flag(electron_objs[i], LV_OBJ_FLAG_HIDDEN)) { lv_anim_del(electron_objs[i], fade_anim_opa_exec_cb); a.var = electron_objs[i]; lv_anim_start(&a); } } for (int i = 0; i < MAX_SHELLS; i++) { if (orbit_objs[i] && lv_obj_is_valid(orbit_objs[i]) && !lv_obj_has_flag(orbit_objs[i], LV_OBJ_FLAG_HIDDEN)) { lv_anim_del(orbit_objs[i], fade_anim_opa_exec_cb); a.var = orbit_objs[i]; lv_anim_start(&a); } } if (nucleus_obj && lv_obj_is_valid(nucleus_obj)) { lv_obj_clear_flag(nucleus_obj, LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_opa(nucleus_obj, LV_OPA_COVER, 0); } }

/** @brief Create display (handles 5 shells, updates info panel including new fields) */
static void create_element_display(uint8_t element_index) {
    ...
    // 1. Update Info Panel Labels (Including new ones)
    char buffer[64];
    ...
    // Update new labels
    lv_label_set_text(label_melt_val, element->melting_point_k);
    lv_label_set_text(label_boil_val, element->boiling_point_k);
    lv_label_set_text(label_density_val, element->density_g_cm3);
    ...
    // 2. Update Animation
    if (!nucleus_obj) { nucleus_obj = lv_obj_create(animation_container); lv_obj_remove_style_all(nucleus_obj); lv_obj_add_style(nucleus_obj, &style_nucleus, 0); lv_obj_set_size(nucleus_obj, NUCLEUS_RADIUS * 2, NUCLEUS_RADIUS * 2); }
    lv_obj_set_pos(nucleus_obj, center_x - NUCLEUS_RADIUS, center_y - NUCLEUS_RADIUS);
    ...
    for (int shell = 0; shell < MAX_SHELLS; shell++) {
        if (element->electrons_per_shell[shell] > 0) {
            ...
            lv_obj_set_pos(orbit_objs[shell], center_x - radius, center_y - radius);
...
static void element_select_event_cb(lv_event_t * e) {
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (index < MAX_ELEMENTS && index != current_element_index) {
           ...
           // Optionally scroll info panel to top
           lv_obj_scroll_to_y(info_panel, 0, LV_ANIM_ON);
        }
    }
}
...
/**
 * @brief Initialize UI: Adds more info fields.
 */
void my_ui_init(void) {
    ...
    lv_style_init(&style_info_panel); lv_style_set_bg_color(&style_info_panel, lv_color_hex(0x182028)); lv_style_set_bg_opa(&style_info_panel, LV_OPA_COVER); lv_style_set_pad_all(&style_info_panel, 8); lv_style_set_radius(&style_info_panel, 5); // Reduced padding slightly
    lv_style_init(&style_info_label_title); lv_style_set_text_color(&style_info_label_title, lv_palette_main(LV_PALETTE_CYAN)); lv_style_set_text_font(&style_info_label_title, &lv_font_montserrat_14); // Smaller title font
    ...
    lv_obj_set_size(main_container_top, lv_pct(100), lv_pct(62));
    ...
    lv_obj_set_size(animation_container, lv_pct(65), lv_pct(100));
    ...
    lv_obj_set_size(info_panel, lv_pct(35), lv_pct(100));
    ...
    lv_obj_set_style_pad_row(info_panel, 3, 0); // Reduce space between label rows
    ...
    lv_obj_set_height(button_container, lv_pct(35));
    ...
    center_x = anim_w / 2;
    center_y = anim_h / 2; // Vertically center animation
    ...
    // --- Create Info Panel Labels (Including new ones) ---
    ...
    // Melting Point Labels
    label_melt_lbl = lv_label_create(info_panel); lv_obj_add_style(label_melt_lbl, &style_info_label_static, 0); lv_label_set_text(label_melt_lbl, "Melting Point:");
    label_melt_val = lv_label_create(info_panel); lv_obj_add_style(label_melt_val, &style_info_label_value, 0); lv_label_set_text(label_melt_val, "-");

    // Boiling Point Labels
    label_boil_lbl = lv_label_create(info_panel); lv_obj_add_style(label_boil_lbl, &style_info_label_static, 0); lv_label_set_text(label_boil_lbl, "Boiling Point:");
    label_boil_val = lv_label_create(info_panel); lv_obj_add_style(label_boil_val, &style_info_label_value, 0); lv_label_set_text(label_boil_val, "-");

    // Density Labels
    label_density_lbl = lv_label_create(info_panel); lv_obj_add_style(label_density_lbl, &style_info_label_static, 0); lv_label_set_text(label_density_lbl, "Density:");
    label_density_val = lv_label_create(info_panel); lv_obj_add_style(label_density_val, &style_info_label_value, 0); lv_label_set_long_mode(label_density_val, LV_LABEL_LONG_WRAP); lv_obj_set_width(label_density_val, lv_pct(100)); lv_label_set_text(label_density_val, "-");

    // Config Labels (Moved lower)
    ...
    LV_LOG_USER("Custom UI Initialized: 44 Elements, Added Physical Properties.");
}